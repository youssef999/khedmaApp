...
/// Container for all the information necessary to build a MyStepper widget's
/// forward and backward controls for any given step.
///
/// Used by [MyStepper.controlsBuilder].
@immutable
class ControlsDetails {
  /// Creates a set of details describing the MyStepper.
...
class MyStep {
  ...
  /// Only [MyStepperType.horizontal], Optional widget that appears under the [title].
...
///
/// {@tool dartpad}
/// An example the shows how to use the [MyStepper], and the [MyStepper] UI
/// appearance.
///
...
class MyStepper extends StatefulWidget {
  ...
  ///                child: Text('Continue to MyStep ${details.stepIndex + 1}'),
  ///              ),
  ///              TextButton(
  ///                onPressed: details.onStepCancel,
  ///                child: Text('Back to MyStep ${details.stepIndex - 1}'),
  ///              ),
  ///            ],
  ///          );
  ///       },
  ...
  /// The elevation of this stepper's [Material] when [type] is [MyStepperType.horizontal].
...
class _MyStepperState extends State<MyStepper> with TickerProviderStateMixin {
  ...
  bool _isLabel() {
    for (final MyStep step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
  ...
  Widget _buildVerticalControls(int stepIndex) {
    ...
    return Container(
      ...
      child: ConstrainedBox(
        ...
        child: Row(
          // The Material spec no longer includes a MyStepper widget. The continue
